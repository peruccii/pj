...
const getAlunobyMatricula = function (matriculaAluno) {
    ...
    alunos.forEach(item => {
        if (item.matricula == matricula) {
            listaStatus.push({ nome : item.nome, foto : item.foto, materias : []})
            item.curso.forEach(itemCurso => {
                itemCurso.disciplinas.forEach(itemCursoDisciplina => {
                    listaStatus[0].materias.push(
                        {
                            nome: itemCursoDisciplina.nome,
                            media: itemCursoDisciplina.media,
                            status: itemCursoDisciplina.status
                        }
                    )
    ...
    listaStatusjson.aluno = listaStatus
